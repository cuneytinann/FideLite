...
A full-FIDE chess arbiter in 1,240 bytes of plain JavaScript — board, interface and all.
The rule engine on its own is 1,199.
...
Castling, en passant, choice of promotion piece, stalemate, insufficient material, threefold and fivefold repetition, the 50- and 75-move rules, draw by agreement, flag fall and resignation are all implemented in full. The one article left out is **the blocked-position case of 5.2.2**: in positions where the material is sufficient but mate is nonetheless impossible, the engine does not declare the draw early. That detection is not unsolved — [CHA-Solver](https://chasolver.org/) decides it with mathematical certainty — but its completeness is measured rather than proven, and it is a search engine in its own right: neither the guarantee nor the size fits a 1,199-byte arbiter. I am building my own approach in [perfect-arbiter](https://cuneytinann.github.io/perfect-arbiter/); the target is not 100% but the blocked positions actually met in play. [The full argument is on the site.](https://www.fidelite.art/#dead)

Every file under `builds/` runs on its own — no library, no install, no server. Download an HTML file and double-click it.

The three JavaScript files there are the rule engine without a front end: `engine.js` (1,199 bytes), `engine_2x.js` (the same rules, +13 bytes, twice the speed) and `engine_onlyPerft.js` (move generation only, for the test suite). The site reads them live and documents them in full — [structure and driver API](https://www.fidelite.art/#engine), [line by line](https://www.fidelite.art/#flow), [the speed measurements](https://www.fidelite.art/#speed).
...
| Variable map — 32 names, shadowings, free letters | [/#names](https://www.fidelite.art/#names) |
...
| Build comparison and `engine_2x` measurements | [/#builds](https://www.fidelite.art/#builds) · [/#speed](https://www.fidelite.art/#speed) |
...
The build currently on the site, `dom_2000elo.html`, is the first of these: **4,057 bytes**, one file. Of that, 1,760 bytes are the arbiter and the interface — the same core as `dom_base` — and the remaining **2,297 are the bot**. In testing it held its own against Stockfish set to 2000. On the site it has only a short row in the builds table for now; neither its architecture nor the measurement method has been written up.
...
Tam FIDE kurallı satranç hakemi, 1.240 bayt saf JavaScript — tahtası ve arayüzüyle birlikte.
Kural motoru tek başına 1.199 bayt.
...
Rok, geçerken alma, seçimli terfi, pat, yetersiz materyal, üçlü ve beşli tekrar, 50/75 hamle, karşılıklı anlaşma, süre ve terk eksiksiz uygulanır. Uygulanmayan tek madde **5.2.2'nin kilitli pozisyon hâli**: materyal yeterliyken matın yine de imkânsız olduğu bloke pozisyonlarda motor beraberliği erkenden ilan etmez. Bu tespit çözümsüz değil — [CHA-Solver](https://chasolver.org/) matematiksel kesinlikle karar veriyor — ama tamlığı kanıtlanmış değil, ölçülmüş; üstelik kendi başına bir arama motoru: ne verdiği garanti ne de boyutu 1.199 baytlık bir hakeme sığıyor. Kendi yaklaşımımı [perfect-arbiter](https://cuneytinann.github.io/perfect-arbiter/) reposunda kuruyorum; hedef %100 değil, pratikte karşılaşılan kilitli pozisyonlar. [Gerekçenin tamamı sitede.](https://www.fidelite.art/tr#dead)
...
Oradaki üç JavaScript dosyası motorun önyüzsüz hâli: `engine.js` (1.199 bayt), `engine_2x.js` (aynı kurallar, +13 bayt, iki kat hız) ve `engine_onlyPerft.js` (yalnız hamle üretimi, test süiti için). Site bu dosyaları canlı okuyor ve ayrıntısıyla belgeliyor — [yapı ve sürücü API'si](https://www.fidelite.art/tr#engine), [satır satır](https://www.fidelite.art/tr#flow), [hız ölçümleri](https://www.fidelite.art/tr#speed).
...
| Değişken haritası — 32 isim, gölgelemeler, boşta harfler | [/tr#names](https://www.fidelite.art/tr#names) |
...
| Sürüm karşılaştırması ve `engine_2x` ölçümleri | [/tr#builds](https://www.fidelite.art/tr#builds) · [/tr#speed](https://www.fidelite.art/tr#speed) |
...
Şu an sitede duran `dom_2000elo.html` bunun ilk örneği: **4.057 bayt**, tek dosya. 1.760 baytı hakem ve arayüz — `dom_base` ile aynı çekirdek — kalan **2.297 baytı bot**. Ölçümlerde Stockfish'in 2000 seviyesiyle başa baş güç gösterdi. Sitede şimdilik yalnızca sürümler tablosunda kısa bir satırı var; ne mimarisi ne de ölçüm yöntemi anlatılmış değil.
